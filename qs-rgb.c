...
void InitI2CMaster()
{
	...
	// From forum post http://e2e.ti.com/support/microcontrollers/tiva_arm/f/908/t/169023.aspx?pi239031349=1
	//
    ...
    // Again from forum post.
    //ROM_SysCtlDelay(10000);

    // Poll the INA219 a few times.  See what we get.
    //
    unsigned uIteration;
    unsigned long ulDataReceived = 0;
    for(uIteration = 0; uIteration < 10; ++uIteration)
    {
    	while(I2CMasterBusy(I2C0_MASTER_BASE)) {} // Poll for completion.  This will later need to be interrupt/event driven.

    	// First need to tell (write) the INA219 that we want to read.
    	//
    	I2CMasterSlaveAddrSet(I2C0_MASTER_BASE, 0x40, false);

    	// Access the bus voltage register -- 0x02.  (Instead seeing what's in the config register.  It works!)
    	//
    	I2CMasterDataPut(I2C0_MASTER_BASE, 0x02 /*0*/);
    	I2CMasterControl(I2C0_MASTER_BASE, I2C_MASTER_CMD_SINGLE_SEND);
    	while(I2CMasterBusy(I2C0_MASTER_BASE)) {} // Poll for completion.  This will later need to be interrupt/event driven.

		// Prepare to do a read from address 0x40
		//
		I2CMasterSlaveAddrSet(I2C0_MASTER_BASE, 0x40, true);

		I2CMasterControl(I2C0_MASTER_BASE, I2C_MASTER_CMD_BURST_RECEIVE_START);
		while(I2CMasterBusy(I2C0_MASTER_BASE)) {} // Poll for completion.  This will later need to be interrupt/event driven.

		//unsigned uError = I2CMasterErr(I2C0_MASTER_BASE);
		//if(uError != I2C_MASTER_ERR_NONE)
		//{
		//}
		ulDataReceived = I2CMasterDataGet(I2C0_MASTER_BASE) << 8;

		I2CMasterControl(I2C0_MASTER_BASE, I2C_MASTER_CMD_BURST_RECEIVE_FINISH /*I2C_MASTER_CMD_BURST_RECEIVE_CONT*/);
		while(I2CMasterBusy(I2C0_MASTER_BASE)) {} // Poll for completion.  This will later need to be interrupt/event driven.
		ulDataReceived |= I2CMasterDataGet(I2C0_MASTER_BASE);
    }
}
...
main(void)
{
    ...
    // reference.  TODO BW : Why 40MHz?  Can this part go faster?  Like 80?
    //
    ROM_SysCtlClockSet(SYSCTL_SYSDIV_5 | SYSCTL_USE_PLL | SYSCTL_XTAL_16MHZ |
                       SYSCTL_OSC_MAIN);

    unsigned long uClock = SysCtlClockGet(); // 40,000,000 (40Mhz).
